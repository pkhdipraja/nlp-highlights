...
While the model proposed by [Kalchbrenner et al.] works quite well in for rescoring translation hypotheses from SMT system and computing perplexity of reference translations, using CNN as encoder means that the ordering of the words are not preserved. In an almost similar manner to this approach, [Cho et al.] attempt to map source sentence to a fixed vector representation then back to the target sentence, but with 2 RNNs as encoder and decoder. In their work..

\section*{The Seq2Seq Model}
%Cho and Kalchbrenner didnt attempt direct translation

\section*{Improvement and Recent Advances} % & related work. also include problems with Seq2Seq?
