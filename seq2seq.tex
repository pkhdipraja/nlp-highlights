...
Intuitively, the attention mechanism can be perceived as a soft alignment between the words in a source sentence and target translation. It allows the model to search and focus only on important information that is relevant for generation of the target word. Furthermore, it is possible to use attention mechanism without any recurrent network units to perform sequence modelling, using the Transformer architecture \citep{NIPS2017_7181}.%transformer easier to be parallelized compared to rnn?
...
aside MT, show and tell, text summarization, question answering
